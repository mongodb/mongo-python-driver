...
class _SocketHolder(object):
    ...
    def discard(self, reuse=True):
        if self.sock is not None:
            ...
            if reuse and self.reusable and self.pid == os.getpid() and len(self.sockets) < self.pool_size:
...
class _Pool(threading.local):
    ...
    # Non thread-locals
    __slots__ = ["pid", "pool_size", "network_timeout", "sockets", "forklock"]
    ...
    def sock(self):
        if self.holder is not None:
            return self.holder.sock
        return None

    def __new__(cls, pool_size, network_timeout):
        # __new__ is called only once, so we can initialize non thread-locals
        self = threading.local.__new__(cls, pool_size, network_timeout)
        self.pid = os.getpid()
        self.pool_size = pool_size
        self.network_timeout = network_timeout
        self.sockets = []
        self.forklock = threading.Lock()
        return self
    ...
    def get_socket(self, host, port):
        ...
        if pid != self.pid:
            ...
            try:
                ...
                self.forklock.release()

        holder = self.holder
        if holder is None or pid != holder.pid:
            self.holder = holder = _SocketHolder(self)

        if holder.reusable and holder.sock is not None:
            holder.reusable = False
            return (holder.sock, True)

        try:
            holder.attach(self.sockets.pop())
            return (holder.sock, True)
        except IndexError:
            holder.attach(self.connect(host, port))
            return (holder.sock, False)
