...
def _build_credentials_tuple(mech, source, user, passwd, extra, database):
    ...
    if mech == "GSSAPI":
        ...
        return _MongoCredential(mech, "$external", user, passwd, props, None)
        ...
        return _MongoCredential(mech, "$external", user, None, None, None)
        ...
        return _MongoCredential(mech, "$external", user, passwd, aws_props, None)
    elif mech == "PLAIN":
        source_database = source or database or "$external"
        return _MongoCredential(mech, source_database, user, passwd, None, None)
    else:
        source_database = source or database or "admin"
        if passwd is None:
            raise ConfigurationError("A password is required.")
        return _MongoCredential(mech, source_database, user, passwd, None, _Cache())
