...
from pymongo.network_layer import NetworkingInterface, PyMongoProtocol
...
def _configured_socket_interface(address: _Address, options: PoolOptions) -> NetworkingInterface:
    """Given (host, port) and PoolOptions, return a configured NetworkingInterface.
    ...
    try:
        # We have to pass hostname / ip address to wrap_socket
        # to use SSLContext.check_hostname.
        transport, protocol = asyncio.get_running_loop().create_connection(  # type: ignore[call-overload]
    ...
    return NetworkingInterface((transport, protocol))
