...
class _SrvResolver:
    ...
    def _resolve_uri(self, is_polling: bool) -> resolver.Answer:
        try:
            results = _resolve(
    ...
    def _get_srv_response_and_hosts(
        ...
        results = self._resolve_uri(is_polling)
    ...
    def get_hosts(self) -> list[tuple[str, Any]]:
        _, nodes = self._get_srv_response_and_hosts(is_polling=False)
        return nodes

    def get_hosts_and_min_ttl(self) -> tuple[list[tuple[str, Any]], int]:
        results, nodes = self._get_srv_response_and_hosts(is_polling=True)
        rrset = results.rrset
