...
def _parse_ssl_options(options):
    ...
    ssl_kwarg_keys = [k for k in options if k.startswith("ssl_") and options[k]]
    if use_ssl is False and ssl_kwarg_keys:
        raise ConfigurationError(
            "ssl has not been enabled but the "
            "following ssl parameters have been set: "
            "%s. Please set `ssl=True` or remove." % ", ".join(ssl_kwarg_keys)
        )
