...
class MasterSlaveConnection(BaseObject):
    ...
    def _send_message(self, message, with_last_error=False,
                      _connection_to_use=None, wtimeout_is_error=True):
        ...
        if _connection_to_use is None or _connection_to_use == -1:
            return self.__master._send_message(
                message, with_last_error, wtimeout_is_error=wtimeout_is_error)
        return self.__slaves[_connection_to_use]._send_message(
            message, with_last_error, check_primary=False,
            wtimeout_is_error=wtimeout_is_error)
