...
async def async_sendall(socket: socket.socket, buf: bytes, flags: int = 0) -> None:
    socket.setblocking(False)
    timeout = socket.gettimeout()
    socket.settimeout(0.0)
...
async def async_receive_message(
    ...
    length, _, response_to, op_code = _UNPACK_HEADER(
        await _async_receive_data_on_socket(conn, 16, deadline)
    )
...
async def _async_receive_data_on_socket(
    ...
    while bytes_read < length:
        try:
            await async_wait_for_read(conn, deadline)
