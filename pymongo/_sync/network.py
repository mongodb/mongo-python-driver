...
def async_send(socket: socket.socket, buf: bytes, flags: int = 0) -> int:
...
def sendall(socket: socket.socket, buf: bytes, flags: int = 0) -> None:
    socket.setblocking(False)
    timeout = socket.gettimeout()
    socket.settimeout(0.0)
    loop = asyncio.get_event_loop()
    try:
        asyncio.wait_for(loop.sock_sendall(socket, buf), timeout=timeout)
    finally:
        socket.settimeout(timeout)
        socket.setblocking(True)
...
def receive_message(
    ...
    length, _, response_to, op_code = _UNPACK_HEADER(
        _async_receive_data_on_socket(conn, 16, deadline)
    )
...
def async_wait_for_read(conn: Connection, deadline: Optional[float]) -> None:
...
def _async_receive_data_on_socket(
    ...
    while bytes_read < length:
        try:
            async_wait_for_read(conn, deadline)
