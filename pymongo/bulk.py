...
class _Bulk(object):
    ...
    def _execute_command(
        self, generator, write_concern, session, sock_info, op_id, retryable, full_result
    ):
    ...
    def execute_insert_no_results(self, sock_info, run, op_id, acknowledged):
        ...
        cmd = SON(
            [("insert", self.collection.name), ("ordered", self.ordered), ("writeConcern", concern)]
        )
        if self.bypass_doc_val and sock_info.max_wire_version >= 4:
            cmd["bypassDocumentValidation"] = True
    ...
    def execute_command_no_results(self, sock_info, generator):
        ...
        try:
            self._execute_command(
                generator, write_concern, None, sock_info, op_id, False, full_result
            )
...
class BulkUpsertOperation(object):
    ...
    def update_one(self, update):
        ...
        """
        self.__bulk.add_update(
            self.__selector, update, multi=False, upsert=True, collation=self.__collation
        )
    ...
    def update(self, update):
        ...
        """
        self.__bulk.add_update(
            self.__selector, update, multi=True, upsert=True, collation=self.__collation
        )
    ...
    def replace_one(self, replacement):
        ...
        """
        self.__bulk.add_replace(
            self.__selector, replacement, upsert=True, collation=self.__collation
        )
