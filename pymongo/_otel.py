...
def _resolve_tracing_options(tracing_options: _UnresolvedTracingOptions) -> TracingOptions:
    ...
    per command. An explicit client value wins, including an ``enabled`` of
    ``False``, which forces tracing off, or a ``query_text_max_length`` of 0,
    which turns ``db.query.text`` off.
    """
    ...
    if max_length is None:
        try:
            ...
            max_length = 0
    enabled = tracing_options["enabled"]
    return {
        "enabled": enabled if enabled is not None else _env_truthy(_OTEL_ENABLED_ENV),
