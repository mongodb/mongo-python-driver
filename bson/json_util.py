...
from bson.py3compat import PY3, integer_types, iteritems, string_type, text_type
...
def _json_convert(obj, json_options=DEFAULT_JSON_OPTIONS):
    ...
    if hasattr(obj, "iteritems") or hasattr(obj, "items"):  # PY3 support
        return SON(((k, _json_convert(v, json_options)) for k, v in iteritems(obj)))
...
def default(obj, json_options=DEFAULT_JSON_OPTIONS):
    ...
    if isinstance(obj, bool):
        return obj
    if json_options.json_mode == JSONMode.CANONICAL and isinstance(obj, integer_types):
        if -(2**31) <= obj < 2**31:
