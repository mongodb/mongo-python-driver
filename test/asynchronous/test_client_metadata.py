...
class TestClientMetadataProse(AsyncIntegrationTest):
    ...
    async def test_1_test_that_the_driver_updates_metadata(self):
    ...
    async def test_1_test_that_the_driver_updates_metadata_platform_none(self):
        client = await self.async_rs_or_single_client(
            "mongodb://" + self.server.address_string,
            maxIdleTimeMS=1,
            driver=DriverInfo("library", "1.2", "Library Platform"),
    ...
    async def test_1_test_that_the_driver_updates_metadata_version_none(self):
        client = await self.async_rs_or_single_client(
            "mongodb://" + self.server.address_string,
            maxIdleTimeMS=1,
            driver=DriverInfo("library", "1.2", "Library Platform"),
    ...
    async def test_1_test_that_the_driver_updates_metadata_platform_version_none(self):
        client = await self.async_rs_or_single_client(
            "mongodb://" + self.server.address_string,
            maxIdleTimeMS=1,
            driver=DriverInfo("library", "1.2", "Library Platform"),
    ...
    async def test_2_multiple_successive_metadata_updates(self):
        client = await self.async_rs_or_single_client(
    ...
    async def test_2_multiple_successive_metadata_updates_platform_none(self):
        client = await self.async_rs_or_single_client(
            "mongodb://" + self.server.address_string,
            maxIdleTimeMS=1,
        )
    ...
    async def test_2_multiple_successive_metadata_updates_version_none(self):
        client = await self.async_rs_or_single_client(
            "mongodb://" + self.server.address_string,
            maxIdleTimeMS=1,
        )
    ...
    async def test_2_multiple_successive_metadata_updates_platform_version_none(self):
        client = await self.async_rs_or_single_client(
            "mongodb://" + self.server.address_string,
            maxIdleTimeMS=1,
        )
    ...
    async def test_duplicate_driver_name_no_op(self):
        ...
        name, version, platform, _ = await self.send_ping_and_get_metadata(client, True)
        await asyncio.sleep(0.005)
        client.append_metadata(DriverInfo("framework", None, None))
        new_name, new_version, new_platform, _ = await self.send_ping_and_get_metadata(client, True)
        self.assertEqual(new_name, name)
        self.assertEqual(new_version, version)
        self.assertEqual(new_platform, platform)

    async def test_9_handshake_documents_include_backpressure(self):
    ...
    async def test_10_entries_in_driver_name_and_driver_version_correspond_by_index(self):
        cases = [
