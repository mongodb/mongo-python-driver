...
class AsyncTestCollection(AsyncIntegrationTest):
    ...
    async def test_create_indexes(self):
        ...
        with self.assertRaises(TypeError):
            await db.coll.create_indexes("foo")  # type: ignore[arg-type]
        with self.assertRaises(TypeError):
            await db.coll.create_indexes(["foo"])  # type: ignore[list-item]
        self.assertRaises(TypeError, IndexModel, 5)
        self.assertRaises(ValueError, IndexModel, [])

        await db.coll.drop_indexes()
        await db.create_collection("coll")
        self.assertEqual(len(await db.coll.index_information()), 1)

        await db.coll.create_indexes([IndexModel("hello")])
        await db.coll.create_indexes([IndexModel([("hello", DESCENDING), ("world", ASCENDING)])])

        # Tuple instead of list.
        await db.coll.create_indexes([IndexModel((("world", ASCENDING),))])

        self.assertEqual(len(await db.coll.index_information()), 4)

        await db.coll.drop_indexes()
        names = await db.coll.create_indexes(
        ...
        self.assertEqual(len(await db.coll.index_information()), 1)
        await db.coll.create_indexes([IndexModel("hello")])
        self.assertIn("hello_1", await db.coll.index_information())
        ...
        await db.coll.drop()
        await db.coll.insert_one({"a": 1})
        await db.coll.insert_one({"a": 1})
        with self.assertRaises(DuplicateKeyError):
            await db.coll.create_indexes([IndexModel("a", unique=True)])
    ...
    async def test_create_index(self):
        ...
        with self.assertRaises(TypeError):
            await db.coll.create_index(5)  # type: ignore[arg-type]
        ...
        await db.coll.create_index("hello")
        await db.coll.create_index([("hello", DESCENDING), ("world", ASCENDING)])

        # Tuple instead of list.
        await db.coll.create_index((("world", ASCENDING),))

        self.assertEqual(len(await db.coll.index_information()), 4)

        await db.coll.drop_indexes()
        ix = await db.coll.create_index(
        ...
        self.assertEqual(len(await db.coll.index_information()), 1)
        await db.coll.create_index([("hello", DESCENDING), ("world", ASCENDING)])
        self.assertIn("hello_-1_world_1", await db.coll.index_information())

        await db.coll.drop_indexes()
        await db.coll.create_index([("hello", DESCENDING), ("world", ASCENDING)], name=None)
        self.assertIn("hello_-1_world_1", await db.coll.index_information())
        ...
        await db.coll.insert_one({"a": 1})
        with self.assertRaises(DuplicateKeyError):
            await db.coll.create_index("a", unique=True)

        with self.write_concern_collection() as coll:
            await coll.create_index([("hello", DESCENDING)])

        await db.coll.create_index(["hello", "world"])
        await db.coll.create_index(["hello", ("world", DESCENDING)])
        await db.coll.create_index({"hello": 1}.items())  # type:ignore[arg-type]

    async def test_drop_index(self):
        db = self.db
        await db.coll.drop_indexes()
        await db.coll.create_index("hello")
        name = await db.coll.create_index("goodbye")

        self.assertEqual(len(await db.coll.index_information()), 3)
        ...
        if async_client_context.version < Version(8, 3, -1):
            ...
            await db.coll.drop_index(name)
        self.assertEqual(len(await db.coll.index_information()), 2)
        self.assertIn("hello_1", await db.coll.index_information())
        ...
        self.assertEqual(len(await db.coll.index_information()), 3)
        self.assertEqual(name, "goodbye_1")
        await db.coll.drop_index([("goodbye", ASCENDING)])
        self.assertEqual(len(await db.coll.index_information()), 2)
        self.assertIn("hello_1", await db.coll.index_information())
    ...
    async def test_list_indexes(self):
        ...
        indexes = await (await db.coll.list_indexes()).to_list()
        ...
        await db.coll.create_index([("hello", DESCENDING), ("world", ASCENDING)], unique=True)
        indexes = await (await db.coll.list_indexes()).to_list()
    ...
    async def test_index_info(self):
        ...
        await db.create_collection("coll")
        self.assertEqual(len(await db.coll.index_information()), 1)
        self.assertIn("_id_", await db.coll.index_information())

        await db.coll.create_index("hello")
        self.assertEqual(len(await db.coll.index_information()), 2)
        self.assertEqual(
            (await db.coll.index_information())["hello_1"]["key"], [("hello", ASCENDING)]
        )
        ...
        self.assertEqual(
            ...
            (await db.coll.index_information())["hello_-1_world_1"]["key"],
        )
        self.assertEqual(True, (await db.coll.index_information())["hello_-1_world_1"]["unique"])

    async def test_index_geo2d(self):
        db = self.db
        await db.coll.drop_indexes()
        self.assertEqual("loc_2d", await db.coll.create_index([("loc", GEO2D)]))
        index_info = (await db.coll.index_information())["loc_2d"]
    ...
    async def test_index_haystack(self):
        ...
        await db.coll.drop()
        _id = (
            await db.coll.insert_one({"pos": {"long": 34.2, "lat": 33.3}, "type": "restaurant"})
        ).inserted_id
        await db.coll.insert_one({"pos": {"long": 34.2, "lat": 37.3}, "type": "restaurant"})
        await db.coll.insert_one({"pos": {"long": 59.1, "lat": 87.2}, "type": "office"})
        await db.coll.create_index([("pos", "geoHaystack"), ("type", ASCENDING)], bucketSize=1)
    ...
    async def test_index_text(self):
        db = self.db
        await db.coll.drop_indexes()
        self.assertEqual("t_text", await db.coll.create_index([("t", TEXT)]))
        index_info = (await db.coll.index_information())["t_text"]
    ...
    async def test_index_2dsphere(self):
        db = self.db
        await db.coll.drop_indexes()
        self.assertEqual("geo_2dsphere", await db.coll.create_index([("geo", GEOSPHERE)]))

        for dummy, info in (await db.coll.index_information()).items():
    ...
    async def test_index_hashed(self):
        db = self.db
        await db.coll.drop_indexes()
        self.assertEqual("a_hashed", await db.coll.create_index([("a", HASHED)]))
    ...
    async def test_index_sparse(self):
        db = self.db
        await db.coll.drop_indexes()
        await db.coll.create_index([("key", ASCENDING)], sparse=True)
        self.assertTrue((await db.coll.index_information())["key_1"]["sparse"])

    async def test_index_background(self):
        db = self.db
        await db.coll.drop_indexes()
        await db.coll.create_index([("keya", ASCENDING)])
        await db.coll.create_index([("keyb", ASCENDING)], background=False)
        await db.coll.create_index([("keyc", ASCENDING)], background=True)
        self.assertNotIn("background", (await db.coll.index_information())["keya_1"])
        self.assertFalse((await db.coll.index_information())["keyb_1"]["background"])
        self.assertTrue((await db.coll.index_information())["keyc_1"]["background"])

    async def _drop_dups_setup(self, db):
        await db.drop_collection("coll")
        await db.coll.insert_one({"i": 1})
        await db.coll.insert_one({"i": 2})
        await db.coll.insert_one({"i": 2})  # duplicate
        await db.coll.insert_one({"i": 3})
    ...
    async def test_index_dont_drop_dups(self):
        ...
        async def _test_create():
            await db.coll.create_index([("i", ASCENDING)], unique=True, dropDups=False)

        with self.assertRaises(DuplicateKeyError):
            await _test_create()

        # Duplicate wasn't dropped
        self.assertEqual(4, await db.coll.count_documents({}))

        # Index wasn't created, only the default index on _id
        self.assertEqual(1, len(await db.coll.index_information()))
    ...
    async def test_index_filter(self):
        ...
        with self.assertRaises(OperationFailure):
            await db.coll.create_index("x", partialFilterExpression=5)
        with self.assertRaises(OperationFailure):
            await db.coll.create_index("x", partialFilterExpression={"x": {"$asdasd": 3}})
        with self.assertRaises(OperationFailure):
            await db.coll.create_index("x", partialFilterExpression={"$and": 5})
        ...
        await db.coll.insert_one({"x": 5, "a": 2})
        await db.coll.insert_one({"x": 6, "a": 1})

        # Operations that use the partial index.
        explain = await db.coll.find({"x": 6, "a": 1}).explain()
        ...
        explain = await db.coll.find({"x": {"$gt": 1}, "a": 1}).explain()
        stage = self.get_plan_stage(explain["queryPlanner"]["winningPlan"], "IXSCAN")
        self.assertEqual("x_1", stage.get("indexName"))
        self.assertTrue(stage.get("isPartial"))

        explain = await db.coll.find({"x": 6, "a": {"$lte": 1}}).explain()
        stage = self.get_plan_stage(explain["queryPlanner"]["winningPlan"], "IXSCAN")
        self.assertEqual("x_1", stage.get("indexName"))
        self.assertTrue(stage.get("isPartial"))

        # Operations that do not use the partial index.
        explain = await db.coll.find({"x": 6, "a": {"$lte": 1.6}}).explain()
        stage = self.get_plan_stage(explain["queryPlanner"]["winningPlan"], "COLLSCAN")
        self.assertNotEqual({}, stage)
        explain = await db.coll.find({"x": 6}).explain()
        stage = self.get_plan_stage(explain["queryPlanner"]["winningPlan"], "COLLSCAN")
        self.assertNotEqual({}, stage)

        # Test drop_indexes.
        await db.coll.drop_index("x_1")
        explain = await db.coll.find({"x": 6, "a": 1}).explain()
        stage = self.get_plan_stage(explain["queryPlanner"]["winningPlan"], "COLLSCAN")
        self.assertNotEqual({}, stage)
    ...
    async def test_field_selection(self):
        ...
        doc = await anext(db.coll.find({}, ["_id"]))
        self.assertEqual(list(doc), ["_id"])
        doc = await anext(db.coll.find({}, ["a"]))
        l = list(doc)
        l.sort()
        self.assertEqual(l, ["_id", "a"])
        doc = await anext(db.coll.find({}, ["b"]))
        l = list(doc)
        l.sort()
        self.assertEqual(l, ["_id", "b"])
        doc = await anext(db.coll.find({}, ["c"]))
        l = list(doc)
        l.sort()
        ...
        doc = await anext(db.coll.find({}, {"a": False, "b": 0}))
        l = list(doc)
        l.sort()
        self.assertEqual(l, ["_id", "c"])

        doc = await anext(db.coll.find({}, {"_id": False}))
        l = list(doc)
    ...
    async def test_options(self):
        db = self.db
        await db.drop_collection("coll")
        await db.create_collection("coll", capped=True, size=4096)
        result = await db.coll.options()
    ...
    async def test_insert_one(self):
        ...
        result = await db.coll.insert_one(document)
        ...
        self.assertIsNotNone(await db.coll.find_one({"_id": document["_id"]}))
        self.assertEqual(1, await db.coll.count_documents({}))
        ...
        self.assertTrue(result.acknowledged)
        self.assertIsNotNone(await db.coll.find_one({"_id": document["_id"]}))
        self.assertEqual(2, await db.coll.count_documents({}))
        ...
        async def async_lambda():
            return await db.coll.count_documents({}) == 2
    ...
    async def test_insert_many(self):
        ...
        result = await db.coll.insert_many(docs)
        ...
        for doc in docs:
            ...
            self.assertEqual(1, await db.coll.count_documents({"_id": _id}))
        self.assertTrue(result.acknowledged)
        ...
        self.assertIsInstance(result, InsertManyResult)
        self.assertFalse(result.acknowledged)
        self.assertEqual(20, await db.coll.count_documents({}))
    ...
    async def test_insert_many_invalid(self):
        ...
        with self.assertRaisesRegex(TypeError, "documents must be a non-empty list"):
            await db.coll.insert_many(1)  # type: ignore[arg-type]

        with self.assertRaisesRegex(TypeError, "documents must be a non-empty list"):
            await db.coll.insert_many(RawBSONDocument(encode({"_id": 2})))

    async def test_delete_one(self):
        await self.db.coll.drop()

        await self.db.coll.insert_one({"x": 1})
        await self.db.coll.insert_one({"y": 1})
        await self.db.coll.insert_one({"z": 1})

        result = await self.db.coll.delete_one({"x": 1})
        self.assertIsInstance(result, DeleteResult)
        self.assertEqual(1, result.deleted_count)
        self.assertTrue(result.acknowledged)
        self.assertEqual(2, await self.db.coll.count_documents({}))

        result = await self.db.coll.delete_one({"y": 1})
        self.assertIsInstance(result, DeleteResult)
        self.assertEqual(1, result.deleted_count)
        self.assertTrue(result.acknowledged)
        self.assertEqual(1, await self.db.coll.count_documents({}))

        db = self.db.client.get_database(self.db.name, write_concern=WriteConcern(w=0))
        result = await db.coll.delete_one({"z": 1})
        self.assertIsInstance(result, DeleteResult)
        self.assertRaises(InvalidOperation, lambda: result.deleted_count)
        self.assertFalse(result.acknowledged)

        async def lambda_async():
            return await db.coll.count_documents({}) == 0
    ...
    async def test_delete_many(self):
        ...
        result = await self.db.coll.delete_many({"x": 1})
        self.assertIsInstance(result, DeleteResult)
        self.assertEqual(2, result.deleted_count)
        self.assertTrue(result.acknowledged)
        self.assertEqual(0, await self.db.coll.count_documents({"x": 1}))

        db = self.db.client.get_database(self.db.name, write_concern=WriteConcern(w=0))
        result = await db.coll.delete_many({"y": 1})
        self.assertIsInstance(result, DeleteResult)
        self.assertRaises(InvalidOperation, lambda: result.deleted_count)
        self.assertFalse(result.acknowledged)
    ...
    async def test_write_large_document(self):
        ...
        with self.assertRaises(OperationFailure):
            await self.db.coll.insert_one({"foo": max_str})
        with self.assertRaises(OperationFailure):
            await self.db.coll.replace_one({}, {"foo": max_str}, upsert=True)
        with self.assertRaises(OperationFailure):
            await self.db.coll.insert_many([{"x": 1}, {"foo": max_str}])
        await self.db.coll.insert_many([{"foo": half_str}, {"foo": half_str}])

        await self.db.coll.insert_one({"bar": "x"})
        ...
        await self.db.coll.replace_one({"bar": "x"}, {"bar": "x" * (max_size - 32)})

    async def test_insert_bypass_document_validation(self):
        db = self.db
        await db.coll.drop()
        await db.create_collection("coll", validator={"a": {"$exists": True}})
        db_w0 = self.db.client.get_database(self.db.name, write_concern=WriteConcern(w=0))

        # Test insert_one
        with self.assertRaises(OperationFailure):
            await db.coll.insert_one({"_id": 1, "x": 100})
        result = await db.coll.insert_one({"_id": 1, "x": 100}, bypass_document_validation=True)
        self.assertIsInstance(result, InsertOneResult)
        self.assertEqual(1, result.inserted_id)
        result = await db.coll.insert_one({"_id": 2, "a": 0})
        self.assertIsInstance(result, InsertOneResult)
        ...
        result = await db.coll.insert_many(docs, bypass_document_validation=True)
        self.assertIsInstance(result, InsertManyResult)
        ...
        for doc in docs:
            ...
            self.assertIsInstance(_id, int)
            self.assertIn(_id, result.inserted_ids)
            self.assertEqual(1, await db.coll.count_documents({"x": doc["x"]}))
        self.assertTrue(result.acknowledged)
        ...
        for doc in docs:
            ...
            self.assertIsInstance(_id, int)
            self.assertIn(_id, result.inserted_ids)
            self.assertEqual(1, await db.coll.count_documents({"a": doc["a"]}))
        self.assertTrue(result.acknowledged)
    ...
    async def test_replace_bypass_document_validation(self):
        ...
        # Test replace_one
        await db.coll.insert_one({"a": 101})
        with self.assertRaises(OperationFailure):
            await db.coll.replace_one({"a": 101}, {"y": 1})
        self.assertEqual(0, await db.coll.count_documents({"y": 1}))
        self.assertEqual(1, await db.coll.count_documents({"a": 101}))
        await db.coll.replace_one({"a": 101}, {"y": 1}, bypass_document_validation=True)
        self.assertEqual(0, await db.coll.count_documents({"a": 101}))
        self.assertEqual(1, await db.coll.count_documents({"y": 1}))
        await db.coll.replace_one({"y": 1}, {"a": 102})
        self.assertEqual(0, await db.coll.count_documents({"y": 1}))
        self.assertEqual(0, await db.coll.count_documents({"a": 101}))
        self.assertEqual(1, await db.coll.count_documents({"a": 102}))

        await db.coll.insert_one({"y": 1}, bypass_document_validation=True)
        with self.assertRaises(OperationFailure):
            await db.coll.replace_one({"y": 1}, {"x": 101})
        self.assertEqual(0, await db.coll.count_documents({"x": 101}))
        self.assertEqual(1, await db.coll.count_documents({"y": 1}))
        await db.coll.replace_one({"y": 1}, {"x": 101}, bypass_document_validation=True)
        self.assertEqual(0, await db.coll.count_documents({"y": 1}))
        self.assertEqual(1, await db.coll.count_documents({"x": 101}))
        await db.coll.replace_one({"x": 101}, {"a": 103}, bypass_document_validation=False)
        self.assertEqual(0, await db.coll.count_documents({"x": 101}))
    ...
    async def test_update_bypass_document_validation(self):
        ...
        # Test update_one
        with self.assertRaises(OperationFailure):
            await db.coll.update_one({"z": 5}, {"$inc": {"z": -10}})
        self.assertEqual(0, await db.coll.count_documents({"z": -5}))
        self.assertEqual(1, await db.coll.count_documents({"z": 5}))
        await db.coll.update_one({"z": 5}, {"$inc": {"z": -10}}, bypass_document_validation=True)
        self.assertEqual(0, await db.coll.count_documents({"z": 5}))
        self.assertEqual(1, await db.coll.count_documents({"z": -5}))
        await db.coll.update_one({"z": -5}, {"$inc": {"z": 6}}, bypass_document_validation=False)
        self.assertEqual(1, await db.coll.count_documents({"z": 1}))
        self.assertEqual(0, await db.coll.count_documents({"z": -5}))

        await db.coll.insert_one({"z": -10}, bypass_document_validation=True)
        with self.assertRaises(OperationFailure):
            await db.coll.update_one({"z": -10}, {"$inc": {"z": 1}})
        self.assertEqual(0, await db.coll.count_documents({"z": -9}))
        self.assertEqual(1, await db.coll.count_documents({"z": -10}))
        await db.coll.update_one({"z": -10}, {"$inc": {"z": 1}}, bypass_document_validation=True)
        self.assertEqual(1, await db.coll.count_documents({"z": -9}))
        self.assertEqual(0, await db.coll.count_documents({"z": -10}))
        await db.coll.update_one({"z": -9}, {"$inc": {"z": 9}}, bypass_document_validation=False)
        self.assertEqual(0, await db.coll.count_documents({"z": -9}))
        ...
        # Test update_many
        await db.coll.insert_many([{"z": i} for i in range(3, 101)])
        await db.coll.insert_one({"y": 0}, bypass_document_validation=True)
        with self.assertRaises(OperationFailure):
            await db.coll.update_many({}, {"$inc": {"z": -100}})
        self.assertEqual(100, await db.coll.count_documents({"z": {"$gte": 0}}))
        self.assertEqual(0, await db.coll.count_documents({"z": {"$lt": 0}}))
        self.assertEqual(0, await db.coll.count_documents({"y": 0, "z": -100}))
        await db.coll.update_many(
            {"z": {"$gte": 0}}, {"$inc": {"z": -100}}, bypass_document_validation=True
        )
        self.assertEqual(0, await db.coll.count_documents({"z": {"$gt": 0}}))
        self.assertEqual(100, await db.coll.count_documents({"z": {"$lte": 0}}))
        await db.coll.update_many(
            {"z": {"$gt": -50}}, {"$inc": {"z": 100}}, bypass_document_validation=False
        )
        self.assertEqual(50, await db.coll.count_documents({"z": {"$gt": 0}}))
        self.assertEqual(50, await db.coll.count_documents({"z": {"$lt": 0}}))

        await db.coll.insert_many([{"z": -i} for i in range(50)], bypass_document_validation=True)
        with self.assertRaises(OperationFailure):
            await db.coll.update_many({}, {"$inc": {"z": 1}})
        self.assertEqual(100, await db.coll.count_documents({"z": {"$lte": 0}}))
        self.assertEqual(50, await db.coll.count_documents({"z": {"$gt": 1}}))
        await db.coll.update_many(
            {"z": {"$gte": 0}}, {"$inc": {"z": -100}}, bypass_document_validation=True
        )
        self.assertEqual(0, await db.coll.count_documents({"z": {"$gt": 0}}))
        self.assertEqual(150, await db.coll.count_documents({"z": {"$lte": 0}}))
        await db.coll.update_many(
            {"z": {"$lte": 0}}, {"$inc": {"z": 100}}, bypass_document_validation=False
        )
        self.assertEqual(150, await db.coll.count_documents({"z": {"$gte": 0}}))
        self.assertEqual(0, await db.coll.count_documents({"z": {"$lt": 0}}))
    ...
    async def test_bypass_document_validation_bulk_write(self):
        db = self.db
        await db.coll.drop()
        await db.create_collection("coll", validator={"a": {"$gte": 0}})
        db_w0 = self.db.client.get_database(self.db.name, write_concern=WriteConcern(w=0))
        ...
        await db.coll.bulk_write(ops, bypass_document_validation=True)

        self.assertEqual(3, await db.coll.count_documents({}))
        self.assertEqual(1, await db.coll.count_documents({"a": -11}))
        self.assertEqual(1, await db.coll.count_documents({"a": -1}))
        self.assertEqual(1, await db.coll.count_documents({"a": -9}))
    ...
    async def test_find_by_default_dct(self):
        db = self.db
        await db.coll.insert_one({"foo": "bar"})
        dct = defaultdict(dict, [("foo", "bar")])  # type: ignore[arg-type]
        self.assertIsNotNone(await db.coll.find_one(dct))
    ...
    async def test_find_w_fields(self):
        ...
        doc = await anext(db.coll.find({}, ["mike"]))
    ...
    async def test_fields_specifier_as_dict(self):
        ...
        await db.coll.insert_one({"x": [1, 2, 3], "mike": "awesome"})

        self.assertEqual([1, 2, 3], (await db.coll.find_one())["x"])
        self.assertEqual([2, 3], (await db.coll.find_one(projection={"x": {"$slice": -2}}))["x"])
        self.assertNotIn("x", await db.coll.find_one(projection={"x": 0}))
        self.assertIn("mike", await db.coll.find_one(projection={"x": 0}))

    async def test_find_w_regex(self):
        db = self.db
        await db.coll.delete_many({})

        await db.coll.insert_one({"x": "hello_world"})
        await db.coll.insert_one({"x": "hello_mike"})
        await db.coll.insert_one({"x": "hello_mikey"})
        await db.coll.insert_one({"x": "hello_test"})

        self.assertEqual(len(await db.coll.find().to_list()), 4)
        self.assertEqual(len(await db.coll.find({"x": re.compile("^hello.*")}).to_list()), 4)
        self.assertEqual(len(await db.coll.find({"x": re.compile("ello")}).to_list()), 4)
        self.assertEqual(len(await db.coll.find({"x": re.compile("^hello$")}).to_list()), 0)
        self.assertEqual(len(await db.coll.find({"x": re.compile("^hello_mi.*$")}).to_list()), 2)
    ...
    async def test_unique_index(self):
        ...
        # No error.
        await db.coll.insert_one({"hello": "world"})
        await db.coll.insert_one({"hello": "world"})

        await db.drop_collection("coll")
        await db.coll.create_index("hello", unique=True)
    ...
    async def test_duplicate_key_error(self):
        ...
        await db.coll.insert_one({"_id": 1, "x": 1})
    ...
    async def test_write_error_text_handling(self):
        ...
        await db.coll.create_index("text", unique=True)
        ...
        await db.coll.insert_one({"text": text})
        ...
        with self.assertRaises(DuplicateKeyError):
            await db.coll.replace_one({"_id": ObjectId()}, {"text": text}, upsert=True)

        # Should raise BulkWriteError, not InvalidBSON
        with self.assertRaises(BulkWriteError):
            await db.coll.insert_many([{"text": text}])
    ...
    async def test_error_code(self):
        try:
            await self.db.coll.update_many({}, {"$thismodifierdoesntexist": 1})
    ...
    async def test_index_on_subfield(self):
        ...
        await db.coll.insert_one({"hello": {"a": 4, "b": 5}})
        await db.coll.insert_one({"hello": {"a": 7, "b": 2}})
        await db.coll.insert_one({"hello": {"a": 4, "b": 10}})

        await db.drop_collection("coll")
        await db.coll.create_index("hello.a", unique=True)
    ...
    async def test_replace_one(self):
        ...
        with self.assertRaises(ValueError):
            await db.coll.replace_one({}, {"$set": {"x": 1}})

        id1 = (await db.coll.insert_one({"x": 1})).inserted_id
        ...
        self.assertTrue(result.acknowledged)
        self.assertEqual(1, await db.coll.count_documents({"y": 1}))
        self.assertEqual(0, await db.coll.count_documents({"x": 1}))
        self.assertEqual((await db.coll.find_one(id1))["y"], 1)  # type: ignore

        replacement = RawBSONDocument(encode({"_id": id1, "z": 1}))
        result = await db.coll.replace_one({"y": 1}, replacement, True)
        self.assertIsInstance(result, UpdateResult)
        self.assertEqual(1, result.matched_count)
        self.assertIn(result.modified_count, (None, 1))
        self.assertIsNone(result.upserted_id)
        ...
        self.assertEqual(1, await db.coll.count_documents({"z": 1}))
        self.assertEqual(0, await db.coll.count_documents({"y": 1}))
        self.assertEqual((await db.coll.find_one(id1))["z"], 1)  # type: ignore

        result = await db.coll.replace_one({"x": 2}, {"y": 2}, True)
        self.assertIsInstance(result, UpdateResult)
        ...
        self.assertTrue(result.acknowledged)
        self.assertEqual(1, await db.coll.count_documents({"y": 2}))

        db = db.client.get_database(db.name, write_concern=WriteConcern(w=0))
        result = await db.coll.replace_one({"x": 0}, {"y": 0})
        self.assertIsInstance(result, UpdateResult)
    ...
    async def test_update_one(self):
        ...
        with self.assertRaises(ValueError):
            await db.coll.update_one({}, {"x": 1})

        id1 = (await db.coll.insert_one({"x": 5})).inserted_id
        result = await db.coll.update_one({}, {"$inc": {"x": 1}})
        self.assertIsInstance(result, UpdateResult)
        self.assertEqual(1, result.matched_count)
        self.assertIn(result.modified_count, (None, 1))
        self.assertIsNone(result.upserted_id)
        self.assertTrue(result.acknowledged)
        self.assertEqual((await db.coll.find_one(id1))["x"], 6)  # type: ignore

        id2 = (await db.coll.insert_one({"x": 1})).inserted_id
        result = await db.coll.update_one({"x": 6}, {"$inc": {"x": 1}})
        self.assertIsInstance(result, UpdateResult)
        self.assertEqual(1, result.matched_count)
        self.assertIn(result.modified_count, (None, 1))
        self.assertIsNone(result.upserted_id)
        self.assertTrue(result.acknowledged)
        self.assertEqual((await db.coll.find_one(id1))["x"], 7)  # type: ignore
        self.assertEqual((await db.coll.find_one(id2))["x"], 1)  # type: ignore

        result = await db.coll.update_one({"x": 2}, {"$set": {"y": 1}}, True)
        self.assertIsInstance(result, UpdateResult)
        self.assertEqual(0, result.matched_count)
        self.assertIn(result.modified_count, (None, 0))
        self.assertIsInstance(result.upserted_id, ObjectId)
        ...
        db = db.client.get_database(db.name, write_concern=WriteConcern(w=0))
        result = await db.coll.update_one({"x": 0}, {"$inc": {"x": 1}})
        self.assertIsInstance(result, UpdateResult)
        self.assertRaises(InvalidOperation, lambda: result.matched_count)
        self.assertRaises(InvalidOperation, lambda: result.modified_count)
        self.assertRaises(InvalidOperation, lambda: result.upserted_id)
    ...
    async def test_update_result(self):
        ...
        result = await db.coll.update_one({"x": 0}, {"$inc": {"x": 1}}, upsert=True)
        self.assertEqual(result.did_upsert, True)

        result = await db.coll.update_one({"_id": None, "x": 0}, {"$inc": {"x": 1}}, upsert=True)
        self.assertEqual(result.did_upsert, True)

        result = await db.coll.update_one({"_id": None}, {"$inc": {"x": 1}})
    ...
    async def test_update_many(self):
        ...
        with self.assertRaises(ValueError):
            await db.coll.update_many({}, {"x": 1})

        await db.coll.insert_one({"x": 4, "y": 3})
        await db.coll.insert_one({"x": 5, "y": 5})
        await db.coll.insert_one({"x": 4, "y": 4})

        result = await db.coll.update_many({"x": 4}, {"$set": {"y": 5}})
        self.assertIsInstance(result, UpdateResult)
        self.assertEqual(2, result.matched_count)
        self.assertIn(result.modified_count, (None, 2))
        self.assertIsNone(result.upserted_id)
        self.assertTrue(result.acknowledged)
        self.assertEqual(3, await db.coll.count_documents({"y": 5}))

        result = await db.coll.update_many({"x": 5}, {"$set": {"y": 6}})
        self.assertIsInstance(result, UpdateResult)
        self.assertEqual(1, result.matched_count)
        self.assertIn(result.modified_count, (None, 1))
        self.assertIsNone(result.upserted_id)
        self.assertTrue(result.acknowledged)
        self.assertEqual(1, await db.coll.count_documents({"y": 6}))

        result = await db.coll.update_many({"x": 2}, {"$set": {"y": 1}}, True)
        self.assertIsInstance(result, UpdateResult)
        self.assertEqual(0, result.matched_count)
        self.assertIn(result.modified_count, (None, 0))
        self.assertIsInstance(result.upserted_id, ObjectId)
        ...
        db = db.client.get_database(db.name, write_concern=WriteConcern(w=0))
        result = await db.coll.update_many({"x": 0}, {"$inc": {"x": 1}})
        self.assertIsInstance(result, UpdateResult)
        self.assertRaises(InvalidOperation, lambda: result.matched_count)
        self.assertRaises(InvalidOperation, lambda: result.modified_count)
        self.assertRaises(InvalidOperation, lambda: result.upserted_id)
    ...
    async def test_update_check_keys(self):
        await self.db.drop_collection("coll")
        self.assertTrue(await self.db.coll.insert_one({"hello": "world"}))
        ...
        with self.assertRaises(OperationFailure):
            await self.db.coll.update_one({"hello": "world"}, doc, upsert=True)
        ...
        with self.assertRaises(OperationFailure):
            await self.db.coll.replace_one({"hello": "world"}, doc, upsert=True)

        # Replace with empty document
        self.assertNotEqual(
            0, (await self.db.coll.replace_one({"hello": "world"}, {})).matched_count
        )

    async def test_acknowledged_delete(self):
        db = self.db
        await db.drop_collection("coll")
        await db.coll.insert_many([{"x": 1}, {"x": 1}])
        self.assertEqual(2, (await db.coll.delete_many({})).deleted_count)
        self.assertEqual(0, (await db.coll.delete_many({})).deleted_count)
    ...
    async def test_count_documents(self):
        ...
        self.assertEqual(await db.coll.count_documents({}), 0)
        await db.wrong.insert_many([{}, {}])
        self.assertEqual(await db.coll.count_documents({}), 0)
        await db.coll.insert_many([{}, {}])
        self.assertEqual(await db.coll.count_documents({}), 2)
        await db.coll.insert_many([{"foo": "bar"}, {"foo": "baz"}])
        self.assertEqual(await db.coll.count_documents({"foo": "bar"}), 1)
        self.assertEqual(await db.coll.count_documents({"foo": re.compile(r"ba.*")}), 2)
    ...
    async def test_estimated_document_count(self):
        ...
        self.assertEqual(await db.coll.estimated_document_count(), 0)
        await db.wrong.insert_many([{}, {}])
        self.assertEqual(await db.coll.estimated_document_count(), 0)
        await db.coll.insert_many([{}, {}])
        self.assertEqual(await db.coll.estimated_document_count(), 2)

    async def test_aggregate(self):
        db = self.db
        await db.drop_collection("coll")
        await db.coll.insert_one({"foo": [1, 2]})

        with self.assertRaises(TypeError):
            await db.coll.aggregate("wow")  # type: ignore[arg-type]
    ...
    async def test_aggregation_cursor_validation(self):
        ...
        cursor = await db.coll.aggregate([projection], cursor={})
    ...
    async def test_aggregation_cursor_alive(self):
        ...
        cursor = await self.db.coll.aggregate(pipeline=[], cursor={"batchSize": 2})
        n = 0
        while True:
            await cursor.next()
            n += 1
    ...
    async def test_large_limit(self):
        ...
        await db.coll_large_limit.create_index([("x", 1)])
        my_str = "mongomongo" * 1000

        await db.coll_large_limit.insert_many({"x": i, "y": my_str} for i in range(2000))

        i = 0
        y = 0
        async for doc in db.coll_large_limit.find(limit=1900).sort([("x", 1)]):
            i += 1
            y += doc["x"]
    ...
    async def test_find_kwargs(self):
        ...
        self.assertEqual(10, await db.coll.count_documents({}))

        total = 0
        async for x in db.coll.find({}, skip=4, limit=2):
            total += x["x"]
    ...
    async def test_rename(self):
        ...
        with self.assertRaises(TypeError):
            await db.coll.rename(5)  # type: ignore[arg-type]
        with self.assertRaises(InvalidName):
            await db.coll.rename("")
        with self.assertRaises(InvalidName):
            await db.coll.rename("te$t")
        with self.assertRaises(InvalidName):
            await db.coll.rename(".test")
        with self.assertRaises(InvalidName):
            await db.coll.rename("test.")
        with self.assertRaises(InvalidName):
            await db.coll.rename("tes..t")

        self.assertEqual(0, await db.coll.count_documents({}))
        ...
        await db.coll.insert_one({})
        with self.assertRaises(OperationFailure):
            await db.foo.rename("coll")
        await db.foo.rename("coll", dropTarget=True)
    ...
    async def test_find_one(self):
        ...
        _id = (await db.coll.insert_one({"hello": "world", "foo": "bar"})).inserted_id

        self.assertEqual("world", (await db.coll.find_one())["hello"])
        self.assertEqual(await db.coll.find_one(_id), await db.coll.find_one())
        self.assertEqual(await db.coll.find_one(None), await db.coll.find_one())
        self.assertEqual(await db.coll.find_one({}), await db.coll.find_one())
        self.assertEqual(await db.coll.find_one({"hello": "world"}), await db.coll.find_one())

        self.assertIn("hello", await db.coll.find_one(projection=["hello"]))
        self.assertNotIn("hello", await db.coll.find_one(projection=["foo"]))
        ...
        self.assertIn("hello", await db.coll.find_one(projection=frozenset(["hello"])))
        self.assertNotIn("hello", await db.coll.find_one(projection=frozenset(["foo"])))

        self.assertEqual(["_id"], list(await db.coll.find_one(projection={"_id": True})))
        self.assertIn("hello", list(await db.coll.find_one(projection={})))
        self.assertIn("hello", list(await db.coll.find_one(projection=[])))

        self.assertEqual(None, await db.coll.find_one({"hello": "foo"}))
        self.assertEqual(None, await db.coll.find_one(ObjectId()))

    async def test_find_one_non_objectid(self):
        db = self.db
        await db.drop_collection("coll")

        await db.coll.insert_one({"_id": 5})

        self.assertTrue(await db.coll.find_one(5))
        self.assertFalse(await db.coll.find_one(6))

    async def test_find_one_with_find_args(self):
        db = self.db
        await db.drop_collection("coll")

        await db.coll.insert_many([{"x": i} for i in range(1, 4)])

        self.assertEqual(1, (await db.coll.find_one())["x"])
        self.assertEqual(2, (await db.coll.find_one(skip=1, limit=2))["x"])

    async def test_find_with_sort(self):
        db = self.db
        await db.drop_collection("coll")

        await db.coll.insert_many([{"x": 2}, {"x": 1}, {"x": 3}])

        self.assertEqual(2, (await db.coll.find_one())["x"])
        self.assertEqual(1, (await db.coll.find_one(sort=[("x", 1)]))["x"])
        self.assertEqual(3, (await db.coll.find_one(sort=[("x", -1)]))["x"])

        async def to_list(things):
            return [thing["x"] async for thing in things]

        self.assertEqual([2, 1, 3], await to_list(db.coll.find()))
        self.assertEqual([1, 2, 3], await to_list(db.coll.find(sort=[("x", 1)])))
        self.assertEqual([3, 2, 1], await to_list(db.coll.find(sort=[("x", -1)])))

        with self.assertRaises(TypeError):
            await db.coll.find(sort=5)
        with self.assertRaises(TypeError):
            await db.coll.find(sort="hello")
        with self.assertRaises(TypeError):
            await db.coll.find(sort=["hello", 1])

    # TODO doesn't actually test functionality, just that it doesn't blow up
    async def test_cursor_timeout(self):
        await self.db.coll.find(no_cursor_timeout=True).to_list()
        await self.db.coll.find(no_cursor_timeout=False).to_list()
    ...
    async def test_exhaust(self):
        ...
        with self.assertRaises(InvalidOperation):
            await anext(self.db.coll.find(cursor_type=CursorType.EXHAUST, limit=5))
        ...
        await self.db.coll.insert_many([{"i": i} for i in range(150)])
        ...
        async for _ in client[self.db.name].coll.find(cursor_type=CursorType.EXHAUST):
            pass
        self.assertEqual(1, len(pool.conns))
    ...
    async def test_query_on_query_field(self):
        await self.db.drop_collection("coll")
        await self.db.coll.insert_one({"query": "foo"})
        await self.db.coll.insert_one({"bar": "foo"})

        self.assertEqual(1, await self.db.coll.count_documents({"query": {"$ne": None}}))
        self.assertEqual(1, len(await self.db.coll.find({"query": {"$ne": None}}).to_list()))

    async def test_min_query(self):
        await self.db.drop_collection("coll")
        await self.db.coll.insert_many([{"x": 1}, {"x": 2}])
        await self.db.coll.create_index("x")
    ...
    async def test_numerous_inserts(self):
        ...
        await self.db.coll.insert_many([{} for _ in range(n_docs)])
        self.assertEqual(n_docs, await self.db.coll.count_documents({}))
        await self.db.coll.drop()
    ...
    async def test_drop_indexes_non_existent(self):
        await self.db.drop_collection("coll")
        await self.db.coll.drop_indexes()
    ...
    async def test_find_one_and_write_concern(self):
        ...
        if async_client_context.is_rs:
            c_wc_error = db.get_collection(
                "coll", write_concern=WriteConcern(w=len(async_client_context.nodes) + 1)
            )
            with self.assertRaises(WriteConcernError):
    ...
    def test_bool(self):
        with self.assertRaises(NotImplementedError):
            bool(AsyncCollection(self.db, "coll"))
