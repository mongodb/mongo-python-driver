...
class TestCollectionWCustomType(IntegrationTest):
    ...
    def test_group_w_custom_type(self):
        ...
        self.assertEqual(
            ...
            test.group(["sku", "qty"], {"sku": "b"}, {}, "function (obj, prev) { }"),
        )
    ...
    def test_map_reduce_w_custom_type(self):
        test = self.db.get_collection("test", codec_options=UPPERSTR_DECODER_CODECOPTS)
        ...
        map = Code("function () {" "  emit(this.sku, this.qty);" "}")
        ...
        full_result = test.inline_map_reduce(map, reduce, full_response=True)
