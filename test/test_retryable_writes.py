...
def non_retryable_single_statement_ops(coll):
    return [
        ...
        (coll.find_and_modify, [{}, {"$set": {"a": 1}}], {"writeConcern": {"w": 0}}),
        # Unacknowledged Delete.
        (coll.find_and_modify, [{}, {}], {"remove": True, "writeConcern": {"w": 0}}),
    ]
...
class TestRetryableWrites(IgnoreDeprecationsTest):
    ...
    def setUp(self):
        if (
            client_context.version.at_least(3, 5)
...
class TestRetryableWritesTxnNumber(IgnoreDeprecationsTest):
    ...
    def test_increment_transaction_id_without_sending_command(self):
        ...
        for method, args, kwargs in _retryable_single_statement_ops(client.db.retryable_write_test):
            listener.results.clear()
