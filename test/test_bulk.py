...
class TestBulk(BulkTestBase):
    ...
    def test_array_filters_unsupported(self):
        requests = [
            UpdateMany({}, {"$set": {"y.$[i].b": 5}}, array_filters=[{"i.b": 1}]),
            UpdateOne({}, {"$set": {"y.$[i].b": 2}}, array_filters=[{"i.b": 3}]),
        ]
        for bulk_op in requests:
            self.assertRaises(ConfigurationError, self.coll.bulk_write, [bulk_op])
    ...
    def test_upsert_uuid_standard_subdocuments(self):
        options = CodecOptions(uuid_representation=UuidRepresentation.STANDARD)
        coll = self.coll.with_options(codec_options=options)
        ids = [{"f": i, "f2": uuid.uuid4()} for i in range(3)]
