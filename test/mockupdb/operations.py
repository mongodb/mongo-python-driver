...
if _HAVE_MOCKUPDB:
    ...
    operations = [
        Operation(
            "find_one",
            lambda client: client.db.coll.find_one(),
        ...
        Operation(
            "count_documents",
            lambda client: client.db.coll.count_documents({}),
            reply={"n": 1},
            op_type="may-use-secondary",
            not_master=not_master_reply,
        ),
        Operation(
            ...
            lambda client: client.db.coll.estimated_document_count(),
            reply={"n": 1},
            op_type="may-use-secondary",
            not_master=not_master_reply,
        ),
        Operation(
            "aggregate",
            lambda client: client.db.coll.aggregate([]),
            reply={"cursor": {"id": 0, "firstBatch": []}},
            op_type="may-use-secondary",
            not_master=not_master_reply,
        ),
        Operation(
            "options",
            lambda client: client.db.coll.options(),
            reply={"cursor": {"id": 0, "firstBatch": []}},
        ...
        Operation(
            "listIndexes",
            lambda client: client.db.coll.index_information(),
            reply={"cursor": {"id": 0, "firstBatch": []}},
            op_type="must-use-primary",
            not_master=not_master_reply,
        ),
