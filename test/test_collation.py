...
class TestCollation(IntegrationTest):
    ...
    def test_map_reduce(self):
        self.db.test.map_reduce(
            "function() {}", "function() {}", "output", collation=self.collation
        )
        self.assertCollationInLastCommand()
    ...
    def test_update(self):
        self.db.test.update({"foo": 42}, {"$set": {"foo": "bar"}}, collation=self.collation)
        command = self.listener.results["started"][0].command
    ...
    def test_find_and(self):
        self.db.test.find_and_modify({"foo": 42}, {"$set": {"foo": 43}}, collation=self.collation)
        self.assertCollationInLastCommand()
    ...
    def test_bulk(self):
        ...
        bulk.find({"noCollation": 84}).upsert().update_one({"$set": {"foo": 10}})
        bulk.find({"noCollation": 45}).update({"$set": {"bar": 42}})
        bulk.find({"foo": 24}, collation=self.collation).replace_one({"foo": 42})
        bulk.find({"foo": 84}, collation=self.collation).upsert().update_one({"$set": {"foo": 10}})
        bulk.find({"foo": 45}, collation=self.collation).update({"$set": {"foo": 42}})
        bulk.execute()
    ...
    def test_mixed_bulk_collation(self):
        ...
        bulk.find({"foo": 42}).upsert().update_one({"$set": {"bar": 10}})
