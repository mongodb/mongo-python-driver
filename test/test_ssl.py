...
from pymongo.ssl_support import HAVE_SSL, _ssl, get_ssl_context, validate_cert_reqs
...
class TestClientSSL(unittest.TestCase):
    ...
    def test_no_ssl_module(self):
        ...
        # Implied
        self.assertRaises(ConfigurationError, MongoClient, ssl_certfile=CLIENT_PEM)
    ...
    def test_config_ssl(self):
        ...
        self.assertRaises(ValueError, validate_cert_reqs, "ssl_cert_reqs", 3)
        self.assertRaises(ValueError, validate_cert_reqs, "ssl_cert_reqs", -1)
        self.assertEqual(validate_cert_reqs("ssl_cert_reqs", None), None)
        self.assertEqual(validate_cert_reqs("ssl_cert_reqs", ssl.CERT_NONE), ssl.CERT_NONE)
        self.assertEqual(validate_cert_reqs("ssl_cert_reqs", ssl.CERT_OPTIONAL), ssl.CERT_OPTIONAL)
        self.assertEqual(validate_cert_reqs("ssl_cert_reqs", ssl.CERT_REQUIRED), ssl.CERT_REQUIRED)
        self.assertEqual(validate_cert_reqs("ssl_cert_reqs", 0), ssl.CERT_NONE)
        self.assertEqual(validate_cert_reqs("ssl_cert_reqs", 1), ssl.CERT_OPTIONAL)
        self.assertEqual(validate_cert_reqs("ssl_cert_reqs", 2), ssl.CERT_REQUIRED)
        self.assertEqual(validate_cert_reqs("ssl_cert_reqs", "CERT_NONE"), ssl.CERT_NONE)
        self.assertEqual(validate_cert_reqs("ssl_cert_reqs", "CERT_OPTIONAL"), ssl.CERT_OPTIONAL)
        self.assertEqual(validate_cert_reqs("ssl_cert_reqs", "CERT_REQUIRED"), ssl.CERT_REQUIRED)
...
class TestSSL(IntegrationTest):
    ...
    def test_ssl_pem_passphrase(self):
        ...
        if not hasattr(ssl, "SSLContext") and not _ssl.IS_PYOPENSSL:
            ...
            connected(MongoClient(uri_fmt % (CLIENT_ENCRYPTED_PEM, CA_PEM), **self.credentials))
    ...
    def test_cert_ssl_uri_support(self):
        ...
        #
        uri_fmt = (
            "mongodb://localhost/?ssl=true&ssl_certfile=%s&ssl_cert_reqs"
    ...
    def test_cert_ssl_validation_hostname_matching(self):
        ...
        ctx = get_ssl_context(None, None, None, None, ssl.CERT_NONE, None, False, True)
        if hasattr(ctx, "load_default_certs"):
            self.assertFalse(ctx.check_hostname)
            ctx = get_ssl_context(None, None, None, None, ssl.CERT_NONE, None, True, True)
            self.assertFalse(ctx.check_hostname)
            ctx = get_ssl_context(None, None, None, None, ssl.CERT_REQUIRED, None, False, True)
            self.assertFalse(ctx.check_hostname)
            ctx = get_ssl_context(None, None, None, None, ssl.CERT_REQUIRED, None, True, True)
            if _PY37PLUS:
    ...
    def test_ssl_crlfile_support(self):
        if not hasattr(ssl, "VERIFY_CRL_CHECK_LEAF") or _ssl.IS_PYOPENSSL:
            ...
            uri_fmt = (
                "mongodb://localhost/?ssl=true&" "ssl_ca_certs=%s&serverSelectionTimeoutMS=100"
            )
            connected(MongoClient(uri_fmt % (CA_PEM,), **self.credentials))

            uri_fmt = (
                "mongodb://localhost/?ssl=true&ssl_crlfile=%s"
                "&ssl_ca_certs=%s&serverSelectionTimeoutMS=100"
            )
            with self.assertRaises(ConnectionFailure):
                connected(MongoClient(uri_fmt % (CRL_PEM, CA_PEM), **self.credentials))
    ...
    def test_validation_with_system_ca_certs(self):
        ...
        with self.assertRaises(ConnectionFailure):
            ...
            connected(
                MongoClient("server", ssl=True, serverSelectionTimeoutMS=100, **self.credentials)
            )
        ...
        connected(
            MongoClient("localhost", ssl=True, serverSelectionTimeoutMS=100, **self.credentials)
        )
    ...
    def test_certifi_support(self):
        ...
        ssl_support.HAVE_WINCERTSTORE = False
        try:
            ctx = get_ssl_context(None, None, None, CA_PEM, ssl.CERT_REQUIRED, None, True, True)
    ...
    def test_mongodb_x509_auth(self):
        ...
        ssl_client = MongoClient(
            client_context.pair, ssl=True, ssl_cert_reqs=ssl.CERT_NONE, ssl_certfile=CLIENT_PEM
        )
        ...
        client = MongoClient(uri, ssl=True, ssl_cert_reqs=ssl.CERT_NONE, ssl_certfile=CLIENT_PEM)
        ...
        bad_client = MongoClient(uri, ssl=True, ssl_cert_reqs="CERT_NONE", ssl_certfile=CLIENT_PEM)
