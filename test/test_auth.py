...
class TestSASLPlain(unittest.TestCase):
    ...
    def test_sasl_plain_bad_credentials(self):
        ...
        with ignore_deprecations():
            ...
            # Bad username
            self.assertRaises(
                OperationFailure, client.ldap.authenticate, "not-user", SASL_PASS, SASL_DB, "PLAIN"
            )
            self.assertRaises(OperationFailure, client.ldap.test.find_one)
            self.assertRaises(OperationFailure, client.ldap.test.insert_one, {"failed": True})

            # Bad password
            self.assertRaises(
                OperationFailure, client.ldap.authenticate, SASL_USER, "not-pwd", SASL_DB, "PLAIN"
            )
            self.assertRaises(OperationFailure, client.ldap.test.find_one)
            self.assertRaises(OperationFailure, client.ldap.test.insert_one, {"failed": True})
...
class TestSCRAMSHA1(IntegrationTest):
    ...
    def setUp(self):
        ...
        if client_context.version < Version(2, 7, 7):
            ...
            if "SCRAM-SHA-1" not in cmd_line.get("parsed", {}).get("setParameter", {}).get(
    ...
    def test_scram_sha1(self):
        ...
        with ignore_deprecations():
            ...
            self.assertTrue(
                client.pymongo_test.authenticate("user", "pass", mechanism="SCRAM-SHA-1")
            )
...
class TestSCRAM(IntegrationTest):
    ...
    def test_scram(self):
        ...
        self.assertTrue(client.testscram.authenticate("sha1", "pwd", mechanism="SCRAM-SHA-1"))
        client.testscram.command("dbstats")
        client.testscram.logout()
        self.assertRaises(
            OperationFailure,
        ...
        client.testscram.command("dbstats")
        client.testscram.logout()
        self.assertTrue(client.testscram.authenticate("sha256", "pwd", mechanism="SCRAM-SHA-256"))
        client.testscram.command("dbstats")
        client.testscram.logout()
        self.assertRaises(
            OperationFailure,
        ...
        client.testscram.command("dbstats")
        client.testscram.logout()
        self.assertTrue(client.testscram.authenticate("both", "pwd", mechanism="SCRAM-SHA-256"))
        client.testscram.command("dbstats")
        client.testscram.logout()
        self.assertTrue(client.testscram.authenticate("both", "pwd", mechanism="SCRAM-SHA-1"))
        client.testscram.command("dbstats")
        client.testscram.logout()

        self.assertRaises(OperationFailure, client.testscram.authenticate, "not-a-user", "pwd")
        ...
        if HAVE_STRINGPREP:
            ...
            client_context.create_user(
                "testscram", u"\u2168", u"\u2163", roles=["dbOwner"], mechanisms=["SCRAM-SHA-256"]
            )

            client_context.create_user(
                "testscram", u"IX", u"IX", roles=["dbOwner"], mechanisms=["SCRAM-SHA-256"]
            )

            self.assertTrue(client.testscram.authenticate(u"\u2168", u"\u2163"))
            client.testscram.command("dbstats")
            client.testscram.logout()
            self.assertTrue(
                client.testscram.authenticate(u"\u2168", u"\u2163", mechanism="SCRAM-SHA-256")
            )
            client.testscram.command("dbstats")
            client.testscram.logout()
            ...
            client.testscram.logout()
            self.assertTrue(
                client.testscram.authenticate(u"IX", u"I\u00ADX", mechanism="SCRAM-SHA-256")
            )
            client.testscram.command("dbstats")
            client.testscram.logout()
            ...
            client.testscram.command("dbstats")
            client = rs_or_single_client_noauth(u"mongodb://IX:IX@%s:%d/testscram" % (host, port))
            client.testscram.command("dbstats")

        self.listener.results.clear()
        client = rs_or_single_client_noauth(
            "mongodb://both:pwd@%s:%d/testscram" % (host, port), event_listeners=[self.listener]
        )
        client.testscram.command("dbstats")
        if client_context.version.at_least(4, 4, -1):
        ...
        client = rs_or_single_client_noauth(
            "mongodb://both:pwd@%s:%d/testscram?authMechanism=SCRAM-SHA-1" % (host, port)
        )
        client.testscram.command("dbstats")

        client = rs_or_single_client_noauth(
            "mongodb://both:pwd@%s:%d/testscram?authMechanism=SCRAM-SHA-256" % (host, port)
        )
        client.testscram.command("dbstats")
...
class TestThreadedAuth(IntegrationTest):
    ...
    def test_db_authenticate_threaded(self):
        ...
        client_context.create_user("db", "user", "pass", roles=["dbOwner"])
