...
from test import IntegrationTest, SkipTest, client_context, db_pwd, db_user, unittest
...
from pymongo import ASCENDING, OFF, IndexModel, InsertOne, monitoring
...
class TestSession(IntegrationTest):
    ...
    def _test_ops(self, client, *ops):
        listener = client.options.event_listeners()[0][0]
    ...
    def test_parallel_collection_scan(self):
        ...
        for event in listener.results["started"]:
            self.assertIn(
                "lsid", event.command, "parallel_scan sent no lsid with %s" % (event.command_name,)
            )

            if event.command_name == "getMore":
                cursor_id = event.command["getMore"]
                if cursor_id in cursor_lsids:
                    self.assertEqual(cursor_lsids[cursor_id], event.command["lsid"])
                else:
...
class TestCausalConsistency(unittest.TestCase):
    ...
    def test_reads(self):
        ...
        self._test_reads(lambda coll, session: coll.count(session=session))
        ...
        if not client_context.is_mongos and not client_context.version.at_least(4, 1, 0):
            ...
            self._test_reads(lambda coll, session: scan(coll, session=session))
    ...
    def test_writes(self):
        ...
        if client_context.supports_reindex:
            self._test_writes(lambda coll, session: coll.reindex(session=session))
    ...
    @client_context.require_no_standalone
    @unittest.skipIf(client_context.serverless, "Serverless does not support currentOp")
    def test_writes_do_not_include_read_concern_current_op(self):
        # Not a write, but currentOp also doesn't support readConcern.
        self._test_no_read_concern(lambda coll, session: coll.database.current_op(session=session))

    @client_context.require_no_standalone
    @unittest.skipIf(client_context.serverless, "Serverless does not support mapReduce")
    def test_writes_do_not_include_read_concern_map_reduce(self):
        ...
        if client_context.supports_reindex:
            self._test_no_read_concern(lambda coll, session: coll.reindex(session=session))
...
class TestSessionsMultiAuth(IntegrationTest):
    ...
    def setUp(self):
        ...
        client_context.create_user("pymongo_test", "second-user", "pass", roles=["readWrite"])
    ...
    def test_session_authenticate_multiple(self):
        ...
        self.assertNotIn(
            "lsid",
            event.command,
            "find_one with multi-auth shouldn't have sent lsid with %s" % (event.command_name),
        )
    ...
    def test_explicit_session_logout(self):
        ...
        with client.start_session() as s:
            ...
            err = "Cannot use session after authenticating with different" " credentials"
...
class TestSessionsNotSupported(IntegrationTest):
    ...
    def test_sessions_not_supported(self):
        with self.assertRaisesRegex(ConfigurationError, "Sessions are not supported"):
