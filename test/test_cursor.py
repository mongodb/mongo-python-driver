...
class TestCursor(IntegrationTest):
    ...
    def test_add_remove_option(self):
        ...
        cursor2 = self.db.coll.find(cursor_type=CursorType.TAILABLE_AWAIT).add_option(128)
        ...
        cursor2 = self.db.coll.find().add_option(16)
        self.assertEqual(cursor._query_flags, cursor2._query_flags)
        ...
        cursor2 = self.db.coll.find().add_option(34)
        self.assertEqual(cursor._query_flags, cursor2._query_flags)
        cursor.remove_option(32)
        self.assertEqual(2, cursor._query_flags)
        ...
        cursor2 = self.db.coll.find().add_option(128)
        self.assertEqual(cursor._query_flags, cursor2._query_flags)
        cursor.remove_option(128)
        self.assertEqual(0, cursor._query_flags)
    ...
    def test_add_remove_option_exhaust(self):
        ...
        if client_context.is_mongos:
            ...
            cursor2 = self.db.coll.find().add_option(64)
            self.assertEqual(cursor._query_flags, cursor2._query_flags)
    ...
    def test_maxtime_ms_message(self):
        ...
        with self.assertRaises(OperationFailure) as error:
            db.coll.find_one({"$where": delay(2)}, max_time_ms=1)
    ...
    def test_max_await_time_ms(self):
        ...
        coll = (self.rs_or_single_client(event_listeners=[listener]))[self.db.name].coll
    ...
    def test_hint(self):
        ...
        with self.assertRaises(OperationFailure):
            db.coll.find({"num": 17, "foo": 17}).hint([("num", ASCENDING)]).explain()
        with self.assertRaises(OperationFailure):
            db.coll.find({"num": 17, "foo": 17}).hint([("foo", ASCENDING)]).explain()

        spec: list[Any] = [("num", DESCENDING)]
        _ = db.coll.create_index(spec)

        first = next(db.coll.find())
        self.assertEqual(0, first.get("num"))
        first = next(db.coll.find().hint(spec))
    ...
    def test_hint_by_name(self):
        ...
        db.coll.create_index([("i", DESCENDING)], name="fooindex")
        first = next(db.coll.find())
        self.assertEqual(0, first.get("i"))
        first = next(db.coll.find().hint("fooindex"))
    ...
    def test_limit(self):
        ...
        count = 0
        for _ in db.coll.find().limit(20):
            count += 1
        self.assertEqual(count, 20)

        count = 0
        for _ in db.coll.find().limit(99):
            count += 1
        self.assertEqual(count, 99)

        count = 0
        for _ in db.coll.find().limit(1):
            count += 1
        self.assertEqual(count, 1)

        count = 0
        for _ in db.coll.find().limit(0):
            count += 1
        self.assertEqual(count, 100)

        count = 0
        for _ in db.coll.find().limit(0).limit(50).limit(10):
            count += 1
    ...
    def test_max(self):
        ...
        db.coll.drop()
        j_index = [("j", ASCENDING)]
        db.coll.create_index(j_index)

        db.coll.insert_many([{"j": j, "k": j} for j in range(10)])
        ...
        db.coll.create_index(index_keys)
    ...
    def test_batch_size(self):
        db = self.db
        db.coll.drop()
        db.coll.insert_many([{"x": x} for x in range(200)])
        ...
        cursor_count((db.coll.find()).batch_size(0), 200)
        cursor_count((db.coll.find()).batch_size(1), 200)
        cursor_count((db.coll.find()).batch_size(2), 200)
        cursor_count((db.coll.find()).batch_size(5), 200)
        cursor_count((db.coll.find()).batch_size(100), 200)
        cursor_count((db.coll.find()).batch_size(500), 200)

        cursor_count((db.coll.find()).batch_size(0).limit(1), 1)
        cursor_count((db.coll.find()).batch_size(1).limit(1), 1)
        cursor_count((db.coll.find()).batch_size(2).limit(1), 1)
        cursor_count((db.coll.find()).batch_size(5).limit(1), 1)
        cursor_count((db.coll.find()).batch_size(100).limit(1), 1)
        cursor_count((db.coll.find()).batch_size(500).limit(1), 1)

        cursor_count((db.coll.find()).batch_size(0).limit(10), 10)
        cursor_count((db.coll.find()).batch_size(1).limit(10), 10)
        cursor_count((db.coll.find()).batch_size(2).limit(10), 10)
        cursor_count((db.coll.find()).batch_size(5).limit(10), 10)
        cursor_count((db.coll.find()).batch_size(100).limit(10), 10)
        cursor_count((db.coll.find()).batch_size(500).limit(10), 10)
    ...
    def test_limit_and_batch_size(self):
        db = self.db
        db.coll.drop()
        db.coll.insert_many([{"x": x} for x in range(500)])
    ...
    def test_skip(self):
        ...
        for i in db.coll.find():
            self.assertEqual(i["x"], 0)
            break

        for i in db.coll.find().skip(20):
            self.assertEqual(i["x"], 20)
            break

        for i in db.coll.find().skip(99):
            self.assertEqual(i["x"], 99)
            break

        for i in db.coll.find().skip(1):
            self.assertEqual(i["x"], 1)
            break

        for i in db.coll.find().skip(0):
            self.assertEqual(i["x"], 0)
            break

        for i in db.coll.find().skip(0).skip(50).skip(10):
            self.assertEqual(i["x"], 10)
            break

        for _ in db.coll.find().skip(1000):
            self.fail()
    ...
    def test_sort(self):
        ...
        db.coll.insert_many([{"x": i} for i in unsort])

        asc = [i["x"] for i in db.coll.find().sort("x", ASCENDING)]
        self.assertEqual(asc, list(range(10)))
        asc = [i["x"] for i in db.coll.find().sort("x")]
        self.assertEqual(asc, list(range(10)))
        asc = [i["x"] for i in db.coll.find().sort([("x", ASCENDING)])]
        self.assertEqual(asc, list(range(10)))

        expect = list(reversed(range(10)))
        desc = [i["x"] for i in db.coll.find().sort("x", DESCENDING)]
        self.assertEqual(desc, expect)
        desc = [i["x"] for i in db.coll.find().sort([("x", DESCENDING)])]
        self.assertEqual(desc, expect)
        desc = [i["x"] for i in db.coll.find().sort("x", ASCENDING).sort("x", DESCENDING)]
        self.assertEqual(desc, expect)
        ...
        db.coll.drop()
        for a, b in shuffled:
            db.coll.insert_one({"a": a, "b": b})

        result = [
            (i["a"], i["b"]) for i in db.coll.find().sort([("b", DESCENDING), ("a", ASCENDING)])
        ]
    ...
    def test_where(self):
        ...
        self.assertEqual(3, len(db.coll.find().where("this.x < 3").to_list()))
        self.assertEqual(3, len(db.coll.find().where(Code("this.x < 3")).to_list()))

        code_with_scope = Code("this.x < i", {"i": 3})
        if client_context.version.at_least(4, 3, 3):
            # MongoDB 4.4 removed support for Code with scope.
            with self.assertRaises(OperationFailure):
                db.coll.find().where(code_with_scope).to_list()

            code_with_empty_scope = Code("this.x < 3", {})
            with self.assertRaises(OperationFailure):
                db.coll.find().where(code_with_empty_scope).to_list()
        else:
            self.assertEqual(3, len(db.coll.find().where(code_with_scope).to_list()))

        self.assertEqual(10, len(db.coll.find().to_list()))
        self.assertEqual([0, 1, 2], [a["x"] for a in db.coll.find().where("this.x < 3")])
        self.assertEqual([], [a["x"] for a in db.coll.find({"x": 5}).where("this.x < 3")])
        self.assertEqual([5], [a["x"] for a in db.coll.find({"x": 5}).where("this.x > 3")])
    ...
    def test_get_more(self):
        db = self.db
        db.drop_collection("coll")
        db.coll.insert_many([{"i": i} for i in range(10)])
        self.assertEqual(10, len(db.coll.find().batch_size(5).to_list()))
    ...
    def test_tailable(self):
        ...
        # this doesn't raise...
        db.coll.insert_many([{"x": i} for i in range(4, 7)])
    ...
    def test_distinct(self):
        ...
        self.db.coll.insert_many([{"a": 1}, {"a": 2}, {"a": 2}, {"a": 2}, {"a": 3}])

        distinct = self.db.coll.find({"a": {"$lt": 3}}).distinct("a")
        distinct.sort()
        ...
        self.db.coll.insert_one({"a": {"b": "a"}, "c": 12})
        self.db.coll.insert_one({"a": {"b": "b"}, "c": 8})
        self.db.coll.insert_one({"a": {"b": "c"}, "c": 12})
        self.db.coll.insert_one({"a": {"b": "c"}, "c": 8})

        distinct = self.db.coll.find({"c": 8}).distinct("a.b")
        distinct.sort()
    ...
    def test_with_statement(self):
        self.db.drop_collection("coll")
        self.db.coll.insert_many([{} for _ in range(100)])

        c1 = self.db.coll.find()
        with self.db.coll.find() as c2:
    ...
    def test_command_cursor_to_list(self):
        ...
        c = self.db.coll.aggregate([{"$changeStream": {}}], maxAwaitTimeMS=1)
        self.addCleanup(c.close)
        docs = c.to_list()
    ...
    def test_command_cursor_to_list_length(self):
        db = self.db
        db.drop_collection("coll")
        db.coll.insert_many([{"foo": 1}, {"foo": 2}])
...
class TestRawBatchCursor(IntegrationTest):
    ...
    def test_find_raw_snapshot_reads(self):
        ...
        with client.start_session(snapshot=True) as session:
            db.coll.distinct("x", {}, session=session)
            batches = db.coll.find_raw_batches(session=session).sort("_id").to_list()
        self.assertEqual(1, len(batches))
        self.assertEqual(docs, decode_all(batches[0]))
    ...
    def test_server_error(self):
        with self.assertRaises(OperationFailure) as exc:
            next(self.db.coll.find_raw_batches({"x": {"$bad": 1}}))
    ...
    def test_collation(self):
        self.db.coll.insert_one({})
        next(self.db.coll.find_raw_batches(collation=Collation("en_US")))

    def test_read_concern(self):
        self.db.get_collection("coll", write_concern=WriteConcern(w="majority")).insert_one({})
...
class TestRawBatchCommandCursor(IntegrationTest):
    ...
    def test_aggregate_raw_snapshot_reads(self):
        ...
        with client.start_session(snapshot=True) as session:
            db.coll.distinct("x", {}, session=session)
            batches = (
                db.coll.aggregate_raw_batches([{"$sort": {"_id": 1}}], session=session)
            ).to_list()
        self.assertEqual(1, len(batches))
        self.assertEqual(docs, decode_all(batches[0]))
    ...
    def test_server_error(self):
        ...
        with self.assertRaises(OperationFailure) as exc:
            (
                self.db.coll.aggregate_raw_batches(
                    [
                        {
                            "$sort": {"_id": 1},
                        },
    ...
    def test_get_item(self):
        with self.assertRaises(InvalidOperation):
            (self.db.coll.aggregate_raw_batches([]))[0]

    def test_collation(self):
        next(self.db.coll.aggregate_raw_batches([], collation=Collation("en_US")))
