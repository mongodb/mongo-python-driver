...
from pymongo import ALL, ASCENDING, DESCENDING, OFF
...
class TestCursor(IntegrationTest):
    ...
    def test_count_with_hint(self):
        ...
        self.assertRaises(OperationFailure, collection.find({"i": 1}).hint("BAD HINT").count)
        ...
        self.assertEqual(0, collection.find({"i": 1}).hint([("x", 1)]).count())
    ...
    def test_getitem_slice_index(self):
        ...
        self.assertEqual(5, len(list(self.db.test.find()[long(20) : long(25)])))
    ...
    def test_comment(self):
        ...
        try:
            ...
            op = self.db.system.profile.find(
                {"ns": regex, "op": "command", "command.count": "test", "command.comment": "foo"}
            )
            self.assertEqual(op.count(), 1)

            self.db.test.find().comment("foo").distinct("type")
            op = self.db.system.profile.find(
                {"ns": regex, "op": "command", "command.distinct": "test", "command.comment": "foo"}
            )
            self.assertEqual(op.count(), 1)
        finally:
    ...
    def test_modifiers(self):
        ...
        with ignore_deprecations():
            ...
            cur = c.find(modifiers={"$maxTimeMS": 500, "$comment": "testing"})
            self.assertTrue("$query" in cur._Cursor__query_spec())
            self.assertEqual(cur._Cursor__query_spec()["$comment"], "testing")
            self.assertEqual(cur._Cursor__query_spec()["$maxTimeMS"], 500)
