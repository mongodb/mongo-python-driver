...
from test import SkipTest, qcheck, unittest
...
from bson.py3compat import PY3, StringIO, abc, iteritems, text_type
...
class TestBSON(unittest.TestCase):
    ...
    def check_encode_then_decode(self, doc_class=dict, decoder=decode, encoder=encode):
        ...
        helper({})
        helper({"test": u"hello"})
        self.assertTrue(isinstance(decoder(encoder({"hello": "world"}))["hello"], text_type))
        helper({"mike": -10120})
    ...
    def test_invalid_decodes(self):
        ...
        self.assertRaises(InvalidBSON, list, decode_file_iter(StringIO(b"\x1B")))
    ...
    def test_bad_dbref(self):
        ...
        self.assertEqual(DBRef("collection", id=None), decode(encode(ref_only))["ref"])
    ...
    def test_bad_encode(self):
        if not PY3:
            ...
            # binary subtype 0.
            self.assertRaises(
                InvalidStringData, encode, {"lalala": "\xf4\xe0\xf0\xe1\xc0 Color Touch"}
            )
    ...
    def test_overflow(self):
        ...
        self.assertRaises(OverflowError, encode, {"x": long(9223372036854775808)})

        self.assertTrue(encode({"x": long(-9223372036854775808)}))
        self.assertRaises(OverflowError, encode, {"x": long(-9223372036854775809)})
    ...
    def test_null_character(self):
        ...
        self.assertRaises(InvalidDocument, encode, {"a": re.compile(u"ab\x00c")})
...
class TestCodecOptions(unittest.TestCase):
    ...
    def test_decode_all_defaults(self):
        ...
        doc = {"sub_document": {}, "uuid": uuid.uuid4(), "dt": datetime.datetime.utcnow()}
    ...
    def test_unicode_decode_error_handler(self):
        ...
        dec = decode(invalid_key, CodecOptions(unicode_decode_error_handler="replace"))
        self.assertEqual(dec, {replaced_key: u"foobar"})

        dec = decode(invalid_key, CodecOptions(unicode_decode_error_handler="ignore"))
        self.assertEqual(dec, {ignored_key: u"foobar"})

        self.assertRaises(
            InvalidBSON, decode, invalid_key, CodecOptions(unicode_decode_error_handler="strict")
        )
        ...
        dec = decode(invalid_val, CodecOptions(unicode_decode_error_handler="replace"))
        self.assertEqual(dec, {u"keystr": replaced_val})

        dec = decode(invalid_val, CodecOptions(unicode_decode_error_handler="ignore"))
        self.assertEqual(dec, {u"keystr": ignored_val})

        self.assertRaises(
            InvalidBSON, decode, invalid_val, CodecOptions(unicode_decode_error_handler="strict")
        )
        ...
        dec = decode(invalid_both, CodecOptions(unicode_decode_error_handler="replace"))
        self.assertEqual(dec, {replaced_key: replaced_val})

        dec = decode(invalid_both, CodecOptions(unicode_decode_error_handler="ignore"))
        self.assertEqual(dec, {ignored_key: ignored_val})

        self.assertRaises(
            InvalidBSON, decode, invalid_both, CodecOptions(unicode_decode_error_handler="strict")
        )
