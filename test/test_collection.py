...
class TestCollection(IntegrationTest):
    ...
    def test_index_management_max_time_ms(self):
        if client_context.version[:2] == (3, 4) and client_context.version[2] < 4:
        ...
        try:
            ...
            self.assertRaises(ExecutionTimeout, coll.reindex, maxTimeMS=1)
        finally:
            self.client.admin.command("configureFailPoint", "maxTimeAlwaysTimeOut", mode="off")

    def test_reindex(self):
        if not client_context.supports_reindex:
            raise unittest.SkipTest("reindex is no longer supported by mongos 4.1+")
        db = self.db
        ...
        coll = Collection(self.db, "test", write_concern=WriteConcern(w=100))
    ...
    def test_insert_bypass_document_validation(self):
        ...
        if client_context.version < (3, 6):
            ...
            self.assertRaises(
                OperationFailure, db_w0.test.insert_one, {"y": 1}, bypass_document_validation=True
            )
        else:
    ...
    def test_write_error_text_handling(self):
        ...
        self.assertRaises(DuplicateKeyError, db.test.insert, {"text": text})
        ...
        self.assertRaises(
            DuplicateKeyError, db.test.update, {"_id": ObjectId()}, {"text": text}, upsert=True
        )
    ...
    def test_write_error_unicode(self):
        ...
        if sys.version_info[0] == 2:
            ...
            self.assertIn("E11000 duplicate key error", unicode(ctx.exception))
    ...
    def test_update_with_invalid_keys(self):
        ...
        # Replace
        self.assertRaises(OperationFailure, self.db.test.replace_one, {"hello": "world"}, doc)
        # Upsert
        self.assertRaises(
            OperationFailure, self.db.test.replace_one, {"foo": "bar"}, doc, upsert=True
        )
    ...
    def test_count(self):
        ...
        self.assertEqual(db.test.count({"foo": re.compile(r"ba.*")}), 2)
    ...
    def test_parallel_scan(self):
        ...
        docs = []
        threads = [
            threading.Thread(target=docs.extend, args=(cursor,)) for cursor in coll.parallel_scan(3)
        ]
        for t in threads:
            t.start()
        for t in threads:
        ...
        self.assertEqual(set(range(8000)), set(doc["_id"] for doc in docs))
    ...
    def test_parallel_scan_max_time_ms(self):
        self.client.admin.command("configureFailPoint", "maxTimeAlwaysTimeOut", mode="alwaysOn")
        try:
            self.assertRaises(ExecutionTimeout, self.db.test.parallel_scan, 3, maxTimeMS=1)
        finally:
            self.client.admin.command("configureFailPoint", "maxTimeAlwaysTimeOut", mode="off")
    ...
    def test_map_reduce(self):
        ...
        result = db.test.map_reduce(map, reduce, out={"merge": "mrunittests"})
        self.assertEqual(3, result.find_one({"_id": "cat"})["value"])
        self.assertEqual(1, result.find_one({"_id": "hampster"})["value"])

        result = db.test.map_reduce(map, reduce, out={"reduce": "mrunittests"})
        ...
        result = db.test.map_reduce(
            map, reduce, out=SON([("replace", "mrunittests"), ("db", "mrtestdb")])
        )
        self.assertEqual(3, result.find_one({"_id": "cat"})["value"])
        self.assertEqual(2, result.find_one({"_id": "dog"})["value"])
        self.assertEqual(1, result.find_one({"_id": "mouse"})["value"])
        ...
        full_result = db.test.map_reduce(map, reduce, out="mrunittests", full_response=True)
        ...
        self.assertTrue(result["results"][1]["_id"] in ("cat", "dog", "mouse"))
        ...
        full_result = db.test.inline_map_reduce(map, reduce, full_response=True)
    ...
    def test_array_filters_unsupported(self):
        ...
        with self.assertRaises(ConfigurationError):
            c.update_one({}, {"$set": {"y.$[i].b": 5}}, array_filters=[{"i.b": 1}])
        with self.assertRaises(ConfigurationError):
            c.update_many({}, {"$set": {"y.$[i].b": 5}}, array_filters=[{"i.b": 1}])
        with self.assertRaises(ConfigurationError):
            c.find_one_and_update({}, {"$set": {"y.$[i].b": 5}}, array_filters=[{"i.b": 1}])
    ...
    def test_find_one_and_write_concern(self):
        ...
        if client_context.version.at_least(3, 1, 9, -1):
            c_w0.find_and_modify({"_id": 1}, {"$set": {"foo": "bar"}})
