...
class TestReadPreferencesBase(TestReplicaSetClientBase):
    ...
    def assertReadsFrom(self, expected, **kwargs):
        c = rs_client(**kwargs)
        wait_until(lambda: len(c.nodes - c.arbiters) == self.w, "discovered all nodes")
...
class TestCommandAndReadPreference(TestReplicaSetClientBase):
    ...
    def setUpClass(cls):
        ...
        coll = cls.c.pymongo_test.get_collection("test", write_concern=WriteConcern(w=cls.w))
        coll.insert_one({})
    ...
    def test_group(self):
        with warnings.catch_warnings():
            ...
            self._test_coll_helper(
                True, self.c.pymongo_test.test, "group", {"a": 1}, {}, {}, "function() { }"
            )
    ...
    def test_inline_map_reduce(self):
        self._test_coll_helper(
            True, self.c.pymongo_test.test, "inline_map_reduce", "function() { }", "function() { }"
        )
