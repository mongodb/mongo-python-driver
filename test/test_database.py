...
from bson.py3compat import PY3, string_type, text_type
from bson.regex import Regex
from bson.son import SON
from pymongo import ALL, OFF, SLOW_ONLY, auth, helpers
...
class TestDatabase(IntegrationTest):
    ...
    def test_repr(self):
        self.assertEqual(
            ...
            "Database(%r, %s)" % (self.client, repr(u"pymongo_test")),
        )
    ...
    def test_collection_names(self):
        self._test_collection_names("collection_names", include_system_collections=False)

    def test_list_collection_names(self):
        self._test_collection_names(
            "list_collection_names", filter={"name": {"$regex": r"^(?!system\.)"}}
        )
    ...
    def test_password_digest(self):
        ...
        self.assertTrue(isinstance(auth._password_digest("mike", "password"), text_type))
        self.assertEqual(
            auth._password_digest("mike", "password"), u"cd7e45b3b2767dc2fa9b6b548457ed00"
        )
        self.assertEqual(
            auth._password_digest("mike", "password"), auth._password_digest(u"mike", u"password")
        )
        self.assertEqual(
            auth._password_digest("Gustave", u"Dor\xe9"), u"81e0e2364499209f466e75926a162d73"
        )
    ...
    def test_authenticate_add_remove_user(self):
        ...
        def check_auth(username, password):
            c = rs_or_single_client_noauth(
                username=username, password=password, authSource="pymongo_test"
            )
        ...
        self.assertRaises(TypeError, auth_db.add_user, "user", "password", "True")
        self.assertRaises(
            ConfigurationError, auth_db.add_user, "user", "password", True, roles=["read"]
        )

        with warnings.catch_warnings():
            warnings.simplefilter("error", DeprecationWarning)
            self.assertRaises(DeprecationWarning, auth_db.add_user, "user", "password")
            self.assertRaises(DeprecationWarning, auth_db.add_user, "user", "password", True)

        with ignore_deprecations():
            self.assertRaises(
                ConfigurationError, auth_db.add_user, "user", "password", digestPassword=True
            )
        ...
        self.assertRaises(OperationFailure, check_auth, "mike", "not a real password")
        ...
        if not client_context.version.at_least(3, 7, 2) or HAVE_STRINGPREP:
            ...
            self.assertRaises(OperationFailure, check_auth, "mike", "password")
    ...
    def test_make_user_readonly(self):
        ...
        c = rs_or_single_client_noauth(username="jesse", password="pw", authSource="pymongo_test")
        ...
        self.assertRaises(OperationFailure, c.pymongo_test.collection.insert_one, {})
    ...
    def test_default_roles(self):
        ...
        info = auth_admin.command("usersInfo", "test_default_roles")["users"][0]
    ...
    def test_new_user_cmds(self):
        ...
        db = rs_or_single_client_noauth(
            username="amalia", password="password", authSource="pymongo_test"
        ).pymongo_test
    ...
    def test_authenticate_multiple(self):
        ...
        admin_db_auth.add_user("ro-admin", "pass", roles=["userAdmin", "readAnyDatabase"])

        self.addCleanup(client_context.drop_user, "admin", "ro-admin")
        users_db_auth.add_user("user", "pass", roles=["userAdmin", "readWrite"])
        ...
        if client_context.version.at_least(4, 4, -1):
            ...
            self.assertEqual(
                names, ["saslStart", "saslContinue", "saslStart", "saslContinue", "delete"]
            )
            ...
            self.assertEqual(
                names, ["getnonce", "authenticate", "getnonce", "authenticate", "delete"]
            )
